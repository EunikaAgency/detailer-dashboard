...
const buildMaterialSummaryGroupPayload = ({
  ...
  const groupedMonthEventRecords = filterRecordsByProductGroups(monthEventRecords, productGroups, ["product", "brand"]);
  const groupedMonthModuleTimeRecords = filterRecordsByProductGroups(monthModuleTimeRecords, productGroups, ["brand"]);
  const groupedYearEventRecords = filterRecordsByProductGroups(yearEventRecords, productGroups, ["product", "brand"]);
  ...
  return {
    ...
    movingMonthly: buildMovingMonthlyModules(groupedYearEventRecords, 7),
  };
};
...
async function buildDashboardReportSource(inputFilters = {}) {
  ...
  slideRetentionRows.forEach((row) => {
    ...
    const brand = getBrandLabel(product, details);
    const division = getDivisionLabel(user);
    const canonicalAttachmentLabel = resolveCanonicalMediaLabel(product, {
  ...
  const filterSourceRecords = [...yearEventRecords, ...yearSlideRetentionRecords];
  ...
  return {
    ...
    materialSummaryGroups: MATERIAL_SUMMARY_GROUPS.map((group) =>
      buildMaterialSummaryGroupPayload({
        ...
        yearEventRecords: selectedYearEventRecords,
      })
    ),
  };
}
...
function createDashboardSlidesPayload(source) {
  ...
  return {
    ...
    slideActivityExportRows: buildSlideActivityExportRows(selectedMonthSlideRetentionRecords, selectedMonthMaterialUsageRecords),
    materialOpenExportRows: buildMaterialOpenExportRows(selectedMonthMaterialUsageRecords),
    materialOpenCountExportRows: buildMaterialOpenCountExportRows(selectedMonthMaterialUsageRecords),
  };
}
...
export async function getDashboardReportSection(inputFilters = {}, section = "full") {
  ...
  switch (section) {
    ...
    default:
      ...
      const slidesPayload = createDashboardSlidesPayload(source);
