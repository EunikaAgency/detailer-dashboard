...
const buildUnifiedExportRows = (records) => {
  ...
  records.forEach((record) => {
    ...
    const secondsViewed = Number(
      record?.durationSeconds || Number(record?.durationMinutes || 0) * 60 || Number(record?.durationMs || 0) / 1000 || 0
    );
    const startedAtMs = new Date(record?.startedAt || 0).getTime();
    const endedAtMs = new Date(record?.endedAt || 0).getTime();
    const materialWindowKey = [date, year, month, team, psr, brand, productName, material].join("||");
    const compositeKey = [date, year, month, team, psr, brand, productName, material].join("||");
  ...
  return Array.from(byCompositeKey.values())
    .map((row) => {
      const window = materialWindows.get(row.materialWindowKey) || {};
      const timeOpenedAt = window?.timeOpenedAt || null;
      const timeClosedAt = window?.timeClosedAt || null;
...
export async function getDashboardReport(inputFilters = {}) {
  ...
  slideRetentionRows.forEach((row) => {
    ...
    const productName = normalizeText(product?.name) || normalizeText(row?.presentationTitle) || "Unknown Product";
    const date = endedAt.toISOString().slice(0, 10);
    const userKey = normalizeText(row?.userId) || getRepLabel(user);
    const materialUseKey = [
      date,
      userKey,
      normalizeText(row?.sessionId) || normalizeText(row?.caseId) || normalizeText(row?.deckId) || attachmentLabel,
      productName,
      toMaterialName({ attachment: attachmentLabel, slide: "", brand }),
    ].join("||");
    ...
    yearSlideRetentionRecords.push({
      ...
      deckId: normalizeText(row?.deckId),
      materialUseKey,
      slide: slideLabel,
      slideExportName,
      slideNumber,
