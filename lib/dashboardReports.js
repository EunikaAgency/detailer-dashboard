...
const buildSlideMetricRows = (records, mode = "total", limit = 12) => {
  ...
  records.forEach((record) => {
    ...
    const slidePrefix = slideNumber > 0 ? `Slide ${slideNumber}` : "Slide";
    const slideLabel = slideName ? `${slidePrefix}: ${slideName}` : slidePrefix;
    const key = normalizeText(record?.slideKey) || `${product}||${slideLabel}`;
