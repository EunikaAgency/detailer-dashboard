...
const buildMaterialUsageSessions = (records) => {
  ...
  records.forEach((record) => {
    ...
    const current = byUseKey.get(materialUseKey) || {
      ...
      psr: normalizeText(record?.psr) || "Unknown Rep",
      brand: normalizeText(record?.brand) || "Unknown Brand",
      productName: normalizeText(record?.productName || record?.product) || "Unknown Product",
    ...
    if (!current.productName) current.productName = normalizeText(record?.productName || record?.product) || "Unknown Product";
...
const TD_PRODUCT_LABELS = ["PLETAA", "MUCOSTA", "MEPTIN", "AMINOLEBAN"];
const CNS_PRODUCT_LABELS = ["ABILIFY", "MAINTENA", "REXULTI"];

const buildPercentRowsFromAllowedLabels = (records, keyField, allowedLabels, valueField = "count") => {
  const allowedLookup = new Map(
    allowedLabels.map((label) => [normalizeComparableText(label), label])
  );
  const totals = new Map(allowedLabels.map((label) => [label, 0]));

  records.forEach((record) => {
    const label = normalizeText(record?.[keyField]);
    if (!label) return;
    const canonicalLabel = allowedLookup.get(normalizeComparableText(label));
    if (!canonicalLabel) return;
    incrementMap(totals, canonicalLabel, Number(record?.[valueField] || 0));
  });
  ...
  return Array.from(totals.entries())
    .map(([label, value]) => ({
      ...
      value: Number(((Number(value || 0) / total) * 100).toFixed(1)),
    }))
...
const buildSlideMetricRows = (records, mode = "total", limit = 12) => {
  ...
  records.forEach((record) => {
    const product = normalizeText(record?.product) || "Unknown Product";
    const slideLabel =
      normalizeText(record?.slideExportName || record?.slideTitle || record?.slideLabel || record?.slide) || "Unknown Slide";
    const key = `${product}||${slideLabel}`;
    const current = grouped.get(key) || {
      label: `${product} - ${slideLabel}`,
      totalMinutes: 0,
      views: 0,
    };
...
export async function getDashboardReportV2(inputFilters = {}) {
  ...
  return {
    filters,
    nationalUtilization: {
      tdShareOfVoice: buildPercentRowsFromAllowedLabels(selectedMonthEventRecords, "product", TD_PRODUCT_LABELS),
      cnsShareOfVoice: buildPercentRowsFromAllowedLabels(selectedMonthEventRecords, "product", CNS_PRODUCT_LABELS),
    },
    teamUtilization: {
