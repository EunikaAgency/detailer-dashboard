...
export async function getDashboardReport(inputFilters = {}) {
  ...
  const [logs, slideRetentionRows] = await Promise.all([
    ...
    SlideRetention.find(retentionMatch)
      .select("userId sessionId presentationId caseId deckId presentationTitle deckTitle slideId slideIndex slideNumber slideTitle slideType endedAt durationMs durationMinutes details")
      .lean(),
  ]);
