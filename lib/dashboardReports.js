...
const normalizeWindowToken = (value) => normalizeText(value).toLowerCase();

const buildMaterialWindowKey = ({
  date,
  year,
  month,
  team,
  ...
  [
    normalizeWindowToken(date),
    normalizeWindowToken(year),
    normalizeWindowToken(month),
    normalizeWindowToken(team),
    normalizeWindowToken(psr),
    normalizeWindowToken(brand),
    normalizeWindowToken(productName),
    normalizeWindowToken(deckId),
    normalizeWindowToken(material),
  ].join("||");
...
const buildMaterialUsageSummary = (records) => {
  ...
  records.forEach((record) => {
    ...
    const windowKey = buildMaterialWindowKey(record);
    ...
    byWindow.set(windowKey, current);
  });
...
const buildUnifiedExportRows = (records, materialUsageRecords = []) => {
  ...
  const explicitMaterialUsageByWindow = buildMaterialUsageSummary(materialUsageRecords);
  ...
  records.forEach((record) => {
    ...
    const productName = normalizeText(record?.product) || "Unknown Product";
    ...
    const startedAtMs = new Date(record?.startedAt || 0).getTime();
    const endedAtMs = new Date(record?.endedAt || 0).getTime();
    const materialWindowKey = buildMaterialWindowKey({
      date,
      year,
      month,
      team,
    ...
    const compositeKey = [date, year, month, team, psr, brand, productName, material, slide].join("||");
    ...
    if (Number.isFinite(startedAtMs) && startedAtMs > 0) {
      currentWindow.timeOpenedAt =
        currentWindow.timeOpenedAt && currentWindow.timeOpenedAt < startedAtMs
          ? currentWindow.timeOpenedAt
          : startedAtMs;
    }
    if (Number.isFinite(endedAtMs) && endedAtMs > 0) {
      currentWindow.timeClosedAt =
        currentWindow.timeClosedAt && currentWindow.timeClosedAt > endedAtMs
          ? currentWindow.timeClosedAt
          : endedAtMs;
    }
  ...
  return Array.from(byCompositeKey.values())
    .map((row) => {
      const explicitUsage = explicitMaterialUsageByWindow.get(row.materialWindowKey) || null;
      return {
        constWindow: materialWindows.get(row.materialWindowKey) || {},
        explicitUsage,
        ...row,
        detailingCount:
          explicitUsage?.materialUseKeys?.size || materialUseKeysByMaterial.get(row.materialWindowKey)?.size || 0,
        secondsViewed: Math.round(row.secondsViewed),
      };
    })
    .map(({ constWindow, explicitUsage, materialWindowKey, ...row }) => {
      const timeOpenedAt = explicitUsage?.timeOpenedAt || constWindow?.timeOpenedAt || null;
      const timeClosedAt = explicitUsage?.timeClosedAt || constWindow?.timeClosedAt || null;
...
async function buildDashboardReportSource(inputFilters = {}) {
  ...
  logs.forEach((log) => {
    ...
    rawEvents.forEach((event, eventIndex) => {
      ...
      const productName = normalizeText(product?.name) || normalizeText(details?.productName) || "Unknown Product";
...
export async function getDashboardReport(inputFilters = {}) {
  const {
    ...
    selectedMonthSlideRetentionRecords,
    selectedYearEventRecords,
  } = await buildDashboardReportSource(inputFilters);
...
export async function getDashboardReportV2(inputFilters = {}) {
  const { filters, selectedMonthEventRecords, selectedYearEventRecords } = await buildDashboardReportSource(inputFilters);

  return {
    filters,
    monthly: {
      product: buildTotalRows(selectedMonthEventRecords, "product", 10),
      person: buildTotalRows(selectedMonthEventRecords, "psr", 10),
      team: buildTotalRows(selectedMonthEventRecords, "team", 10),
    },
    yearly: {
