...
const buildUnifiedExportRows = (records) => {
  ...
  records.forEach((record) => {
    ...
    const slide = normalizeText(record?.slideLabel || record?.slide || `Slide ${Number(record?.slideNumber || 0) || 1}`);
    const detailingCount = Math.max(1, Number(record?.views || 1));
    const minutesViewed = Number(record?.durationMinutes || 0);
    const compositeKey = [year, month, team, psr, brand, slide].join("||");
