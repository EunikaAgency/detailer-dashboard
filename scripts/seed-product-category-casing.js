...
const Product =
  mongoose.models.ProductCategoryCapsSeed ||
  mongoose.model("ProductCategoryCapsSeed", ProductSchema);
...
async function main() {
  ...
  const candidates = products
    .map((product) => {
      ...
      const nextCategory = currentCategory.toUpperCase();
      if (!currentCategory || currentCategory === nextCategory) return null;
  ...
  if (!candidates.length) {
    console.log("No product category casing updates needed.");
    await mongoose.connection.close();
    return;
  }

  console.log(`Found ${candidates.length} product(s) requiring category uppercasing.`);
  ...
  const bulkOps = candidates.map((product) => ({
    updateOne: {
      ...
      update: { $set: { category: product.nextCategory } },
    },
  }));
